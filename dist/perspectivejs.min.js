...
/*!
  ...
  */return function(){function t(t,a){if(t&&t.strokeStyle&&a&&a.width&&a.height){var e=document.createElement("canvas");e.width=Math.round(a.width),e.height=Math.round(a.height);var o=e.getContext("2d");o.drawImage(a,0,0,e.width,e.height);var i=document.createElement("canvas");i.width=t.canvas.width,i.height=t.canvas.height;var h=i.getContext("2d");this.ctxd=t,this.cvso=e,this.ctxo=o,this.ctxt=h}}return t.prototype.draw=function(t){for(var a=t.topLeftX,e=t.topLeftY,o=t.topRightX,i=t.topRightY,h=t.bottomRightX,n=t.bottomRightY,s=t.bottomLeftX,r=t.bottomLeftY,c=[Math.sqrt(Math.pow(a-o,2)+Math.pow(e-i,2)),Math.sqrt(Math.pow(o-h,2)+Math.pow(i-n,2)),Math.sqrt(Math.pow(h-s,2)+Math.pow(n-r,2)),Math.sqrt(Math.pow(s-a,2)+Math.pow(r-e,2))],p=this.cvso.width,d=this.cvso.height,f=0,v=0,g=0,l=0;l<4;l++){var m=0;(m=l%2?c[l]/p:c[l]/d)>v&&(f=l,v=m),0==c[l]&&g++}if(!(g>1)){var w=this.ctxo,M=this.ctxt;if(M.clearRect(0,0,M.canvas.width,M.canvas.height),f%2==0){(Y=this.create_canvas_context(p,10)).globalCompositeOperation="copy";for(var u=Y.canvas,x=0;x<d;x+=2){var b=a+(s-a)*(C=x/d),y=e+(r-e)*C,T=o+(h-o)*C,R=i+(n-i)*C,L=Math.atan((R-y)/(T-b)),X=Math.sqrt(Math.pow(T-b,2)+Math.pow(R-y,2))/p;Y.setTransform(1,0,0,1,0,-x),Y.drawImage(w.canvas,0,0),M.translate(b,y),M.rotate(L),M.scale(X,X),M.drawImage(u,0,0),M.setTransform(1,0,0,1,0,0)}}else if(f%2==1){var Y;(Y=this.create_canvas_context(10,d)).globalCompositeOperation="copy";u=Y.canvas;for(var _=0;_<p;_+=2){var C;b=a+(o-a)*(C=_/p),y=e+(i-e)*C,T=s+(h-s)*C,R=r+(n-r)*C,L=Math.atan((b-T)/(R-y)),X=Math.sqrt(Math.pow(T-b,2)+Math.pow(R-y,2))/d;Y.setTransform(1,0,0,1,-_,0),Y.drawImage(w.canvas,0,0),M.translate(b,y),M.rotate(L),M.scale(X,X),M.drawImage(u,0,0),M.setTransform(1,0,0,1,0,0)}}this.ctxd.save(),this.ctxd.drawImage(M.canvas,0,0),this._applyMask(this.ctxd,t),this.ctxd.restore()}},t.prototype.create_canvas_context=function(t,a){var e=document.createElement("canvas");return e.width=t,e.height=a,e.getContext("2d")},t.prototype._applyMask=function(t,a){t.beginPath(),t.moveTo(a.topLeftX,a.topLeftY),t.lineTo(a.topRightX,a.topRightY),t.lineTo(a.bottomRightX,a.bottomRightY),t.lineTo(a.bottomLeftX,a.bottomLeftY),t.closePath(),t.globalCompositeOperation="destination-in",t.fill(),t.globalCompositeOperation="source-over"},t}()}));
//# sourceMappingURL=perspectivejs.min.js.map
